...
export default function App() {
  ...
  const [airfoil, setAirfoil] = useState<AirfoilResponse | null>(null)
  const [analysis, setAnalysis] = useState<AnalysisResponse | null>(null)
  const [loading, setLoading] = useState(false)

  const fetchAirfoil = async () => {
    const res = await fetch(`${API}/api/naca4?digits=${digits}&chord=1&n_points=${nPoints}`)
  ...
  const cpPlot = useMemo(() => {
    ...
    return {
      data: [{ x: s, y: cp, mode: 'lines+markers', name: 'Cp' }] as Plotly.Data[],
      layout: {
    ...
    <div className="grid">
      <div className="panel">
        ...
        <div className="row">
          ...
          <div>
            ...
            <input
              ...
              min={50}
              max={600}
              onChange={(e) => setNPoints(parseInt(e.target.value || '200', 10))}
            />
          </div>
        </div>
        <div className="row">
          <div>
            ...
            <input
              ...
              min={40}
              max={300}
              onChange={(e) => setPanels(parseInt(e.target.value || '120', 10))}
            />
          </div>
          <div>
        ...
        <div className="legend">
          Cp plot uses a simple constant-strength vortex panel method with a Kutta condition. Thin-airfoil CL uses a
          zero-lift α estimated from the camber line.
        </div>
